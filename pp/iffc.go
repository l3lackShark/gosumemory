		...
		if memory.DynamicAddresses.IsReady == true {
			...
			case 0, 1:
				...
				if memory.MenuData.OsuStatus == 2 && memory.GameplayData.Combo.Max > 0 {
					...
					res, err := wiekuCalcCrutch(memory.MenuData.Bm.Path.FullDotOsu, int16(memory.MenuData.Bm.Stats.BeatmapMaxCombo), memory.GameplayData.Hits.H300+memory.GameplayData.Hits.H0, memory.GameplayData.Hits.H100, memory.GameplayData.Hits.H50, 0)
					if err != nil {
						pp.Println(err)
